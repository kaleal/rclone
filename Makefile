...
# Make version suffix -YYYY-MM-DD-HHMMSS-CCCCCCCC (C=Commit) or blank
VERSION_SUFFIX := $(shell git show --no-patch --no-notes --date=format:%Y-%m-%d-%H%M%S --pretty='-beta-%cd-%h' HEAD)
